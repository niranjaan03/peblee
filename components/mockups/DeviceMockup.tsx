...
export default function DeviceMockup() {
  return (
    <div className="w-full h-full flex overflow-hidden rounded-[inherit]">
      {/* Sidebar */}
      <div className="w-[50px] shrink-0 flex flex-col items-center pt-4 gap-3 border-r border-white/5" style={{ background: "#141414" }}>
        {["H", "C", "D", "S"].map((icon, i) => (
          <div key={i} className="w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-bold cursor-pointer"
            style={{
              background: i === 2 ? "rgba(111,151,215,0.2)" : "transparent",
              color: i === 2 ? "#6F97D7" : "rgba(255,255,255,0.25)",
            }}>
            {icon}
          </div>
        ))}
      ...
      {/* Content */}
      <div className="flex-1 flex flex-col p-4" style={{ background: "#0f0f0f" }}>
        <div className="flex items-center justify-between mb-4">
          <div className="text-[13px] font-semibold text-white">Devices</div>
          <div className="flex gap-2">
            {["All", "Online", "Offline"].map((f, i) => (
              <button key={f} className="text-[10px] px-2.5 py-1 rounded-full"
                style={{
        ...
        <div className="flex flex-col gap-2.5">
          {devices.map((d) => (
            <div key={d.name} className="px-4 py-3 rounded-xl border border-white/6 flex items-center gap-3"
